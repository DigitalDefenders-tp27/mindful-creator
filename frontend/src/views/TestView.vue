    ...
    <div v-if="result" class="results-container">
      ...
      <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
        ...
        <div class="p-4 border rounded-md bg-white">
          ...
          <div class="grid grid-cols-3 gap-2 text-center">
            <div class="p-2 bg-green-100 rounded-md">
              <div class="text-2xl font-bold">{{ result.analysis.sentiment.positive_count }}</div>
              <div>Positive</div>
            </div>
            <div class="p-2 bg-gray-100 rounded-md">
              <div class="text-2xl font-bold">{{ result.analysis.sentiment.neutral_count }}</div>
              <div>Neutral</div>
            </div>
            <div class="p-2 bg-red-100 rounded-md">
              <div class="text-2xl font-bold">{{ result.analysis.sentiment.negative_count }}</div>
              <div>Negative</div>
            </div>
          </div>
        </div>
        ...
        <div class="p-4 border rounded-md bg-white">
          ...
          <div class="mb-3">
            <div class="text-2xl font-bold">{{ result.analysis.toxicity.toxic_count }}</div>
            <div>Toxic Comments ({{ result.analysis.toxicity.toxic_percentage.toFixed(1) }}%)</div>
          </div>
